...
<%
	...
	List<pollDTO> list = dao.selectAll_poll();
%>
<!-- Wrapper -->
			<div id="wrapper">
				<!-- Menu -->
					<nav id="Update">	
						<table style="text-align: center;">
							...
							<% for(int i=0; i<list.size();i++){%>
								<tr>
									<td><%=list.get(i).getVote_num() %></td>
									<td><%=list.get(i).getVote_title() %></td>
									<td><%=list.get(i).getId() %></td>
									<td><a href="delete_pollcon?vote_num=<%=list.get(i).getVote_num()%>">투표삭제</a></td>
								</tr>
							<%} %>
						</table>
					</nav>	
