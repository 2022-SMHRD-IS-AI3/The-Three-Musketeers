...
<body>
	<div class="container managergrounp" id="container" name="container">
		...
		<table class="bbsList" summary="">
			...
			<tbody class="body">
				...
				<tr>
					...
					<td><a href="#" target="_self"><%=dto_array.get(i).getBoard_title().toString()%><span
							class="txt_org"></span></a></td>
				...
				<tr>
					...
					<!-- 맨앞으로 이동 --> <%}
                            if(page_count <= 3 && page_count >= 1){/* 1~3 */
                           	 	for(int i = 1; i<=5; i++) {%> <a
						href="board.jsp?page=<%=i %>" class="num_box"><%=i %></a> <%} /* 처음꺼 다뜨기(5개) */ System.out.println("처음꺼 다뜨기(5개)");
						...
						<a href="board.jsp?page=<%=i %>" class="num_box"><%=i %></a> <%} /* 뒤에꺼 다뜨기(5개) */System.out.println("뒤에꺼 다뜨기(5개)");
                            }else{
                            	for(int i = page_count-2; i<= page_count+2; i++) {%>
						<a href="board.jsp?page=<%=i %>" class="num_box"><%=i %></a> <%} /* 자기중심5개까지만뜨기 */System.out.println("자기중심5개까지만뜨기");
                            } 
                            %> <%if(dto_array.size()/10-2 >= page_count && page_count <= dto_array.size()/10+1){ %>
						<a href="board.jsp?page=<%=dto_array.size()/10+1 %>"
						class="arrow radius-left">≫</a>
					<!-- 맨뒤로 이동 --> <%} %>
					</td>
				</tr>
			</tfoot>
		...
		<div class="btn_wrap">
			<a class="btn_org" href="javascript:;"><span class="txt_white">게시글
					작성</span></a>
		</div>
	</div>
